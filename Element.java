...
/**
 * The Element represents the soccer ball element in the game. It extends
 * Polygon
 */
public class Element extends Polygon {
	...
	public void paint(Graphics brush) {
		...
		brush.fillOval(centerX - radius, centerY - radius, 2 * radius, 
				2 * radius);
		...
		for (int i = 0; i < 5; i++) {
			...
			int x2 = (int) (centerX + radius / 3.0 * Math.cos(angle + Math.PI 
					/ 5.0));
			int y2 = (int) (centerY + radius / 3.0 * Math.sin(angle + Math.PI
					/ 5.0));
			int x3 = (int) (centerX + radius / 3.0 * Math.cos(angle - Math.PI 
					/ 5.0));
			int y3 = (int) (centerY + radius / 3.0 * Math.sin(angle - Math.PI 
					/ 5.0));
	...
	public void updatePosition(double xChange, double yChange) {
		this.position.setLocation(this.position.getX() + xChange, 
				this.position.getY() + yChange);
	}
